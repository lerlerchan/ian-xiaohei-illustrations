# Ian Chibi Illustrations

> 把中文文章里的判断、流程、状态和隐喻，变成一张张白底、手绘、cozy 但清爽的正文配图。
>
> 16:9 横版 | Chibi IP | 纯白手绘 | 少量红橙蓝中文批注 | Codex Skill

---

## 这个仓库是什么

Ian Chibi Illustrations 是一个 Codex Skill，用来指导 AI Agent 为中文文章、帖子、博客、Notion 文档和方法论内容生成正文配图。

它不是通用插画 prompt，也不是 PPT 信息图模板。它的核心目标是：先理解文章里的认知锚点，再把其中一个判断、流程、结构、状态或隐喻，变成一张有记忆点的 16:9 手绘解释图。

默认视觉 IP 是 **chibi**：大头小身体（头约占总高 35–45%）、软圆脸部、简洁黑色线稿、cozy tech-life 情绪。Chibi 不是贴纸，也不是站在角落里的装饰物，而是正在认真参与系统运转的可爱工作者。
...
- 想把抽象判断画成具体隐喻的人
- 想要一种比 PPT 信息图更轻、更 cozy、更有个人识别度的配图风格的人
...
- 想要游戏角色设计或 over-rendered 彩色 IP 的人
...
- 每张图的主题、核心意思、结构类型、chibi 动作和中文标注建议
...
- Chibi 必须参与核心动作，不能只是装饰
- Cozy editorial 感：清爽、有趣，但不幼稚、不商业

### Chibi 角色规格（Style Identity）

| 属性 | 规格 |
|------|------|
| 比例 | 大头小身体，头约 35–45% 总高 |
| 脸部 | 大圆眼，圆润脸颊，简洁鼻嘴 |
| 线条 | 全黑线稿，轻微抖动，无彩色填充 |
| 服装 | 宽松卫衣 / 睡衣 / 毛衣（按场景切换） |
| 情绪 | 专注、困意、平静、cozy — 不夸张 |

服装和姿势可以根据场景切换，但比例和线条风格必须保持一致。
...
这些图片是构图和留白风格的参考样例，不是角色模板。新生成的图会使用 chibi IP，构图应从当前文章重新发明。
...
```bash
git clone https://github.com/lerlerchan/ian-xiaohei-illustrations.git
...
```text
Use $ian-xiaohei-illustrations 为这篇中文文章设计并生成 5 张 chibi 风格正文配图。
```
...
每张图写清楚：放在哪段后、主题、核心意思、结构类型、chibi 在做什么、建议中文标注词。
...
```text
Use $ian-xiaohei-illustrations 把下面这篇文章生成 4 张 chibi 风格正文配图。
...
5. 重新发明一个低科技、有趣但成立的物理隐喻
6. 让 chibi 承担核心动作
7. 每张图单独调用图像模型生成
8. 按 QA checklist 检查：白底、留白、chibi 比例、chibi 动作、中文标注、非 PPT 感、非旧案例复刻
9. 保存最终 PNG，并报告用途和路径
...
- Chibi 必须承担核心动作；如果去掉 chibi 画面仍然完全成立，说明 chibi 太装饰了。
- 示例图只用于校准线条密度、留白和颜色克制，不要复刻构图。
- AI 图像模型可能出现错字、幻觉标签、风格漂移或多余标题，生成后需要检查。
- 如果中文错字严重，优先减少标注词并重生成。
- Chibi 角色始终使用黑色线稿，不加彩色填充；颜色只用于批注和路径。
...
这套 chibi 配图 Skill，只是我用 AI 搭建个人生产系统里的一个小工具。
